function sequenceness = compute_replay_tm(data,classifier,nullperms)
...
%     Y = normr(1 ./ (1 + exp(-(X*C.betas'))));
    ...
    for perm = 1:nPerms
        ...
        for ilag = 1:bins
            ...
            tmp = pinv([TM(:,idx) mTM(:,idx) ones(size(TM,1),1)])*Y;
            betas(1,idx,:) = tmp(1:length(idx),:);
            betas(2,idx,:) = tmp(length(idx)+1:length(idx)*2,:);
        end

        betas_TM = reshape(squeeze(betas(1,:,:)),[maxLag nStates^2]);
        betas_mTM = reshape(squeeze(betas(2,:,:)),[maxLag nStates^2]);
        ...
        % Second-level GLM
        SEQ_TM      = pinv(dM) * [betas_TM'];
        SEQ_mTM      = pinv(dM) * [betas_mTM'];
                                  
        % Save to variable
        thisreplay(perm,1,:,:) = SEQ_TM(1:nTransitions,:);
        thisreplay(perm,2,:,:) = SEQ_TM(nTransitions+1:nTransitions*2,:);
        thisreplay(perm,3,:,:) = SEQ_TM(1:nTransitions,:) - SEQ_TM(nTransitions+1:nTransitions*2,:);
        thisreplay(perm,4,:,:) = SEQ_mTM(1:nTransitions,:);
        thisreplay(perm,5,:,:) = SEQ_mTM(nTransitions+1:nTransitions*2,:);
        thisreplay(perm,6,:,:) = SEQ_mTM(1:nTransitions,:) - SEQ_mTM(nTransitions+1:nTransitions*2,:);
