function [cfg, artifact] = ss_artifact_zvalue(cfg, data)
...
trllength     = size(data,3);     % length of each trial
numtrl        = size(data,1);
cfg.artfctdef.zvalue.channel = 1:size(data,2);
sgnind        = 1:size(data,2);
...
opt.trl = [1:trllength:numtrl*trllength; trllength:trllength:numtrl*trllength]';
...
if strcmp(cfg.artfctdef.zvalue.artfctpeak, 'yes')
  ...
  for tt=1:opt.numtrl
    ...
    for rr=1:size(artbegend,1)
      ...
      dssartifact(cnt,1)=max(peaks(cnt)+cfg.artfctdef.zvalue.artfctpeakrange(1)*cfg.Fs,opt.trl(tt,1));
      dssartifact(cnt,2)=min(peaks(cnt)+cfg.artfctdef.zvalue.artfctpeakrange(2)*cfg.Fs,opt.trl(tt,2));
...
artcfg    = opt.artcfg;
trl       = opt.trl;
trlpadsmp = round(artcfg.trlpadding*cfg.Fs);
channel   = opt.channel;
...
% data = preproc(data, '', cfg.Fs, artcfg, [], artcfg.fltpadding, artcfg.fltpadding);

% the string us used as title and printed in the command window
str = sprintf('trial %3d of %d, channel %s', trlop, size(trl,1), num2str(sgnind));
fprintf('showing %s\n', str);
...
xval = ((trl(opt.trlop,1):trl(opt.trlop,2))-trl(opt.trlop,1)+trl(opt.trlop,3))./opt.cfg.Fs;
if trlpadsmp>0
...
if isempty(get(opt.h2, 'children'))
  ...
  vline(xval(  1)+(trlpadsmp-1/opt.cfg.Fs), 'color', [0 0 0],     'displayname', 'vline1');
  vline(xval(end)-(trlpadsmp/opt.cfg.Fs),   'color', [0 0 0],     'displayname', 'vline2');
  data(~artval) = nan;
  ...
  abc2 = axis(opt.h2);
  set(findall(h2children, 'displayname', 'vline1'), 'XData', [1 1]*xval(  1)+(trlpadsmp-1/opt.cfg.Fs));
  set(findall(h2children, 'displayname', 'vline1'), 'YData', abc2(3:4));
  set(findall(h2children, 'displayname', 'vline2'), 'XData', [1 1]*xval(end)-(trlpadsmp/opt.cfg.Fs));
...
if isempty(get(opt.h3, 'children'))
  ...
  hline(opt.threshold, 'color', 'r', 'linestyle', ':', 'displayname', 'threshline');
  vline(xval(  1)+(trlpadsmp-1/opt.cfg.Fs),     'color', [0 0 0],     'displayname', 'vline1b');
  vline(xval(end)-(trlpadsmp/opt.cfg.Fs),       'color', [0 0 0],     'displayname', 'vline2b');
  zval(~artval) = nan;
  ...
  abc = axis(opt.h3);
  set(findall(h3children, 'displayname', 'vline1b'), 'XData', [1 1]*xval(  1)+(trlpadsmp-1/opt.cfg.Fs));
  set(findall(h3children, 'displayname', 'vline1b'), 'YData', abc(3:4));
  set(findall(h3children, 'displayname', 'vline2b'), 'XData', [1 1]*xval(end)-(trlpadsmp/opt.cfg.Fs));
