...
for s = 1:N
    ...
    % we can use the full decision epoch sequenceness already generated during the optimisation stage
    if e == 1 && strcmp(decisionType,'full')
        tmp = load(fullfile(dir_load,subject,[subject '_cl_' num2str(trainTimes(t)) 'ms_' epochs{e} '.mat'])); % loads 'criteria', 'sq', 'lidx'
        this_sq = squeeze(tmp.sq(this_lambda,:,:,:,:,:));
        this_behav = event.table(ismember(event.table.ExpTrial,test.L),:);
        this_behav.Subject = repmat({subject},size(this_behav,1),1);
        behav = [behav; ...
                 this_behav];
    else
        ...
        for trl = 1:nTrls
            ...
            if strcmp(epoch,'decision') % take tails
                for i = 1:2
